...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x, pred):
        ...
        x = self.decoder_with_cross_attention(x, pred)
...
class DecoderWithCrossAttention(nn.Module):
    def __init__(self, config):
        ...
        # l1
        self.crossblock_l1 = TransCrossBlock(self.config, in_ch=2*self.config.base_chan, out_ch=1*self.config.base_chan,
                                             dim_head=1*self.config.base_chan/4)
        self.conv_l1 = ResidualBasicBlock(1 * self.config.base_chan, 1 * self.config.base_chan, stride=1)
        ...
        # l0
        self.conv_l0_1 = ResidualBasicBlock(1 * self.config.base_chan, 8, stride=1)
        self.conv_l0_2 = ResidualBasicBlock(9, 9, stride=1)

    def forward(self, features, pred):
        
        #x5 = self.transblock_l5(features[0])
        
        # l4
        x4 = self.crossblock_l4(features[0], features[1])
        x4 = self.conv_l4(x4)
        ...
        # l2
        x2 = self.crossblock_l2(x3, features[3])
        x2 = self.conv_l2(x2)

        # l1
        x1 = self.crossblock_l1(x2, features[4])
        x1 = self.conv_l1(x1)

        x0 = self.conv_l0_1(x1)
        out = torch.cat([x0, pred], dim=1)
        out = self.conv_l0_2(out)

        #x1 = self.conv_l1_1(x2)
        #out = torch.cat([x1, features[4]], dim=1)
...
class SegmentationHead(nn.Module):
    def __init__(self, config):
        ...
        self.config = config
        self.conv = nn.Conv2d(9, self.config.out_chan, kernel_size=1)
